...
function photozoom_setup() {
	...
	if ( is_admin() && isset( $_GET['activated'] ) ) {
		add_action( 'admin_notices', 'photozoom_activation_notice' );
	}
...
function photozoom_js_scripts() {
	...
	if (! is_admin()) {
		...
		wp_register_script( 'photozoom-scripts', get_template_directory_uri() . '/js/photozoom.js', array( 'jquery' ), '20180914', true );
...
function photozoom_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . /* translators: page number */ sprintf( esc_html__( 'Page %s', 'photozoom' ), max( $paged, $page ) );
	}
